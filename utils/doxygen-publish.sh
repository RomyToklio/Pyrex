...
echo "Generating:"
doxygen Doxyfile && echo "Backup previous version:" && rm -rf ~/pyrexcoin-www-previous && mv "$wwwdir/doc" ~/pyrexcoin-www-previous && cp -ar doc/ "$wwwdir/" && echo "Done, builded and copied to public - the doxygen docs" && echo "size:" && du -Dsh "$wwwdir/" && echo "files:" && find "$wwwdir/" | wc -l
