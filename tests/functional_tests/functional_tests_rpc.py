...
pyrexcoind_base = [builddir + "/bin/pyrexcoind", "--regtest", "--fixed-difficulty", "1", "--offline", "--no-igd", "--p2p-bind-port", "pyrexcoind_p2p_port", "--rpc-bind-port", "pyrexcoind_rpc_port", "--zmq-rpc-bind-port", "pyrexcoind_zmq_port", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--log-level", "1"]
...
for i in range(N_MONERODS):
  command_lines.append([str(18180+i) if x == "pyrexcoind_rpc_port" else str(18280+i) if x == "pyrexcoind_p2p_port" else str(18380+i) if x == "pyrexcoind_zmq_port" else x for x in pyrexcoind_base])
  outputs.append(open(builddir + '/tests/functional_tests/pyrexcoind' + str(i) + '.log', 'a+'))
