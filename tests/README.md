...
Core tests take longer than any other Pyrexcoin tests, due to the high amount of computational work involved in validating core components.
...
To run only Pyrexcoin's core tests (after building):
...
To run only Pyrexcoin's crypto tests (after building):
...
```
pyrexcoind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `pyrexcoin-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Pyrexcoin's hash tests (after building):
...
To run only Pyrexcoin's performance tests (after building):
...
To run only Pyrexcoin's unit tests (after building):
